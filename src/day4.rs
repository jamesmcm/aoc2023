...
pub fn solve_part2(input: &[Card]) -> u32 {
    ...
    for i in 1..=max_id {
        let c = &input[(i - 1) as usize]; // Assume ordered
        ...
        for j in 1..=win_count {
            let this_card_count = *hand.get(&c.id).unwrap();
            hand.entry(c.id + j as u32)
                .and_modify(|e| *e += this_card_count)
                .or_insert(this_card_count);
        }
    }
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11";
        ...
        assert_eq!(solve_part2(&input_generator(inp)), 30);
    }
}
